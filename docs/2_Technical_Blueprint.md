...
* **Applies to Table:** `u_database_table`
...
* **Form Related List:** Added as related list `Approval Request` to default view of `u_database_table`.
...
* **Logic:**
  1. Instantiates record in `u_database_table`.
...
* **Trigger:** After Update when `source_table = u_database_table` and state changes to `approved` or `rejected`.
* **Logic:**
  1. Retrieves target `u_database_table` record via `current.sysapproval`.
